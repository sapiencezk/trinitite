...
#endif
#ifdef M37_A
static const uint8_t M37_A_DATA_PLAN_ID[32] = {
    0x7b,0x8f,0xa6,0x1a,0x5b,0x3e,0x2c,0x7f,
...
#endif
#ifdef M37_IEC_SERVICE
static const uint8_t M37_SERVICE_FORMULA_ID[32] = {
    0x81,0x90,0xcf,0xa0,0x8e,0x25,0xe6,0x5a,
    0x10,0x60,0x1b,0x5b,0x27,0x87,0x9f,0x4e,
    0x21,0x55,0xb0,0xe1,0xf4,0x2b,0x25,0x4d,
    0xf7,0xf0,0x6b,0x46,0xd0,0xbc,0xaf,0xa5,
};
static const uint8_t M37_SERVICE_DATA_PLAN_ID[32] = {
...
#endif
#ifdef M36_TYPED
/* SHA-256 of the canonical M36-T transport plan at the fixed host port base.
 * It is policy data, not a runtime registry; the target still validates the
 * independent typed wire schema and binding before application mutation. */
static const uint8_t M36_DATA_PLAN_ID[32] = {
    0x55,0x82,0x23,0x71,0x99,0xf0,0xed,0x12,
    0x9e,0xc0,0x54,0x4c,0x18,0x0e,0xc8,0x51,
    0x20,0x74,0x1b,0x3e,0x0a,0x3f,0x14,0x45,
    0x96,0xd8,0xaf,0xed,0x46,0x7b,0x69,0x97,
};
#endif
...
{
    if (!p
        || p->generation != 2 || p->manager != M29_MANAGER
        ...
        || !equal_bytes(p->formula,
#ifdef M37_IEC_SERVICE
                        M37_SERVICE_FORMULA_ID,
#else
                        M29_FORMULA_ID,
#endif
                        32)) return 0;
#if defined(M37_IEC_SERVICE)
    if (!equal_bytes(p->data_plan, M37_SERVICE_DATA_PLAN_ID, 32)) return 0;
...
#else
    if (!equal_bytes(p->data_plan, M29_DATA_PLAN_ID, 32)) return 0;
#endif
    return 1;
}
                                       ...
                                       const m29_policy_t *policy)
{
    ...
    return cord_is(tag, "i2-resource-program-v1")
        ...
        && i2_application_surface_tag_matches(
            binding_tag, "m37-iec-service-binding-v1", 26)
        && noun_is_cell(binding_body)
    ...
    return cord_is(tag, "i2-resource-program-v1")
        ...
        && take(binding_noun, &binding_tag, &binding_body)
        && i2_application_surface_tag_matches(
            binding_tag, "m37-a-transport-binding-v1", 26)
        && noun_is_cell(services);
 #else
    return cord_is(tag, "i2-resource-program-v1")
...
static int activate_install(const m29_request_t*r,uint8_t*reserved,uint16_t*reserved_len)
{
    ...
    m29_policy_t policy;
    if(!parse_policy(g_stage,g_policy_bytes,&policy))goto reject;
    if(!policy_bootstrap_valid(&policy)
 ...
 #if defined(M37_IEC_SERVICE)
    if(m37_service_target_prepare_gate(candidate,&identity,capability,&prepared)!=0)
    ...
    if(m37_target_prepare_gate(candidate,&identity,capability,&prepared)!=0)
        return m37_rejected_response(r->operation,direct(828),reserved,reserved_len);
 #elif defined(M36_TYPED)
    if(m36_target_prepare_gate(candidate,&identity,capability,&prepared)!=0)goto reject;
 #else
 ...
 #ifdef M37_IEC_SERVICE
    m37_service_target_publish_gate(prepared,&identity,capability);
 #elif defined(M37_A_R)
...
static int execute(const m29_request_t*r,uint8_t*out,uint16_t*length)
{
    ...
    if(r->op==M29_STATUS){if(!status_body(&body))return 0;result=M29_DONE;}
        ...
        int stopped = 0;
 #ifdef M37_IEC_SERVICE
        stopped = (g_selected ? m37_service_target_set_running(0) : m26_target_set_running(0)) == 0;
 #elif defined(M37_A_R)
 ...
 #else
        stopped = m26_target_set_running(0) == 0;
 #endif
        if(g_lifecycle==M29_RUNNING&&!g_stage_open&&stopped){g_lifecycle=M29_STOPPED;result=M29_DONE;}
        if(result==M29_DONE&&!status_body(&body))return 0;
    }
 ...
 #ifdef M37_IEC_SERVICE
        initialized = g_selected && m37_service_target_init() == 0;
        running = initialized && m29_native_init() == 0 && m37_service_target_set_running(1) == 0;
 #elif defined(M37_A_R)
 ...
 #elif defined(M36_TYPED)
        initialized = g_selected && m36_target_init() == 0;
        running = initialized && m29_native_init() == 0 && m36_target_set_running(1) == 0;
 #else
        initialized = m26_target_init() == 0;
        running = initialized && m29_native_init() == 0 && m26_target_set_running(1) == 0;
 #endif
        if(g_selected&&(g_lifecycle==M29_IDLE||g_lifecycle==M29_STOPPED)&&!g_stage_open&&running){g_lifecycle=M29_RUNNING;result=M29_DONE;if(!status_body(&body))return 0;}
    }
