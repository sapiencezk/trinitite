                                ...
                                unsigned *count)
{
    ...
    unsigned n = 0;
    /* M20's outer noun is immutable program data.  Snapshot ownership still
     * derives from the same flat instances table as M17--M19. */
    if (take(program, &tag, &rest) && m7_cord_eq(tag, "i2-m20-program")) {
        noun base, tables;
