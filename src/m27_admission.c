...
/* Hashes are stored in the canonical little-endian atom order used by the
 * PILL2 identity and admission helpers.  The catalog is intentionally two
 * rows: the boot predecessor and the one authorized successor. */
static const m27_entry_t M27_CATALOG[2] = {
    {
...
{
    return program_hash && (equal(M27_CATALOG[0].program, program_hash)
        || equal(M27_CATALOG[1].program, program_hash));
}
                         ...
                         const i2_admission_catalog_entry_t **out)
{
    ...
    const m27_entry_t *found = 0;
    unsigned matches = 0;
    for (unsigned i = 0; i < 2; i++) {
        const m27_entry_t *entry = &M27_CATALOG[i];
